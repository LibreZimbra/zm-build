...
install_zm_ldap_utilities() {
    for i in Exchange2000.xml Exchange2003.xml Exchange5.5.xml domino.xml novellGroupWise.xml openldap.xml ; do
    ...
    for i in \
            migrate20110615-AddDynlist.pl \
            migrate20110721-AddUnique.pl \
            migrate20111019-UniqueZimbraId.pl \
            migrate20120210-AddSearchNoOp.pl \
            migrate20120507-UniqueDKIMSelector.pl \
            migrate20140728-AddSSHA512.pl \
            migrate20141022-AddTLSBits.pl \
            migrate20150930-AddSyncpovSessionlog.pl \
    ; do
        install_libexec_scripts "zm-ldap-utilities/src/ldap/migration/$i"
    done
